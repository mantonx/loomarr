	...
	repository = &recordedExecutionRepository{
		...
		ReserveFunc: func(ctx context.Context, command HostedCallReservation) (LedgerEvent, error) {
			...
			event := LedgerEvent{ID: command.EventID, RunID: command.RunID, Ordinal: command.Ordinal, Kind: LedgerInferenceReserved, CreatedAt: command.CreatedAt,
				Reserve: &InferenceReserved{EvaluationID: command.EvaluationID, RequestSHA256: command.RequestSHA256, RequestedProvider: command.RequestedProvider, RequestedModel: command.RequestedModel, UpstreamProvider: command.UpstreamProvider, CapabilitySHA256: command.Versions.CapabilitySHA256, PromptSHA256: command.Versions.PromptSHA256, SchemaSHA256: command.Versions.SchemaSHA256, CandidateID: command.CandidateID, Modalities: slices.Clone(command.Modalities), RequestedNanoUSD: command.RequestedNanoUSD, ReservedNanoUSD: reserved, State: reservationState}}
			return event, nil
		},
