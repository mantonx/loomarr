-- +goose Up
-- V67 (§10): append-only current-use filler rights authority.
-- The immutable history retains every reviewed answer. The separate head table is only an
-- atomic compare-and-swap pointer, so replacing a current answer cannot erase its basis.
