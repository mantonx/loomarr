...
func TestRunPublishesCompleteVideoDecisionSummary(t *testing.T) {
	...
	code := run(args, &stdout, &stderr, capabilities{publish: func(config fillerreview.TemporalStructureCompleteShadowDecisionSetConfig) (fillerreview.TemporalStructureShadowDecisionSet, string, error) {
		called = true
...
func TestRunRequiresCompleteVideoDecisionInputs(t *testing.T) {
	var stdout, stderr bytes.Buffer
	if code := run(nil, &stdout, &stderr, capabilities{}); code != 2 || stdout.Len() != 0 || !strings.Contains(stderr.String(), "two complete-video family results") {
